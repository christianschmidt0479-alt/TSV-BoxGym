...
import { getMemberV2ByEmail, updateMemberV2Password } from '@/lib/v2/members/members_v2';
import { verifySession } from '@/lib/v2/auth/session';
import { hashPassword, validatePassword } from '@/lib/v2/auth/password';
...
export async function POST(req: NextRequest) {
  ...
  if (!validatePassword(body.password)) {
    return NextResponse.json({ error: ERR.INVALID_PASSWORD }, { status: 400 });
  }
  const hash = await hashPassword(body.password);
  const updated = await updateMemberV2Password(member.id, hash);
