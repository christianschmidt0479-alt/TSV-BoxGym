...
export function DatenClient({ initialMember }: { initialMember: MemberView }) {
  const [member, setMember] = useState<MemberView>(initialMember)
  const [saving, setSaving] = useState(false)
  const [editMode, setEditMode] = useState(false)
  const [firstName, setFirstName] = useState("")
  const [lastName, setLastName] = useState("")
  const [phone, setPhone] = useState(initialMember.phone === "-" ? "" : initialMember.phone)
  const [birthdate, setBirthdate] = useState(initialMember.birthdate || "")
  const [email, setEmail] = useState(initialMember.email === "-" ? "" : initialMember.email)
