...
export default function MemberAreaPage() {
  ...
  // Gewichtspflicht-Status nach Check-in
  const [requiresWeightEntryToday, setRequiresWeightEntryToday] = useState(false)
  const [weightAlreadyRecordedToday, setWeightAlreadyRecordedToday] = useState(false)
  const [weightEntryValue, setWeightEntryValue] = useState("")
  const [weightEntrySaving, setWeightEntrySaving] = useState(false)
  ...
  function applyMemberSnapshot(snapshot: MemberAreaSnapshot) {
    ...
    // Gewichtspflicht-Status zurücksetzen, wenn ein neues Mitglied geladen wird
    setRequiresWeightEntryToday(false)
    setWeightAlreadyRecordedToday(false)
    setWeightEntryValue("")
    setWeightEntryError("")
  }
  ...
  const nextBaseGroupSession = useMemo(() => {
    ...
    for (let offset = 0; offset <= 7; offset++) {
      ...
      if (matchingSession) {
        return { date: dateString, session: matchingSession }
      }
    }
  ...
  async function loadMemberArea() {
    ...
    try {
      ...
      const snapshot = (await response.json()) as MemberAreaSnapshot & {
        ...
        weight_already_recorded_today?: boolean
      }
      setPrivacyConsentRequired(false)
      setPrivacyAccepted(false)
      setPrivacyError("")
      applyMemberSnapshot(snapshot)
      // Gewichtspflicht-Status auswerten
      setRequiresWeightEntryToday(Boolean(snapshot.requires_weight_entry_today))
      setWeightAlreadyRecordedToday(Boolean(snapshot.weight_already_recorded_today))
      setWeightEntryValue("")
      setWeightEntryError("")
  ...
  // Defensive Guard: memberAttendanceRows darf nie ungeprüft mit .filter verwendet werden
  ...
  const competitionWeightRows = useMemo(() => {
    return safeMemberAttendanceRows.filter((row) => row && typeof row.weight === "number")
  ...
  // Defensive Guard: recentCheckins darf nie ungeprüft mit .length/.map verwendet werden
                ...
                <Card className="rounded-[24px] border-0 shadow-sm">
                              ...
                              {requiresWeightEntryToday && !weightAlreadyRecordedToday && (
                                <Card className="mb-6 border-2 border-blue-400 bg-blue-50">
                                  <CardHeader>
                                    <CardTitle>Gewichtseingabe erforderlich</CardTitle>
                                  </CardHeader>
                                  <CardContent>
                                    <form
                                        ...
                                        try {
                                          const response = await fetch("/api/public/member-checkin", {
                                            ...
                                            body: JSON.stringify({
                                              email: memberAreaEmail.trim().toLowerCase(),
                                              password: memberAreaPin.trim(),
                                              weight: weightEntryValue.trim(),
                                            }),
                                          })
                                          if (!response.ok) {
                                            const msg = await response.text()
                                            setWeightEntryError(msg || "Fehler beim Speichern des Gewichts.")
                                            return
                                          }
                                          setWeightEntryValue("")
                                          setRequiresWeightEntryToday(false)
                                          setWeightAlreadyRecordedToday(true)
                                        } catch (err) {
                                          setWeightEntryError("Fehler beim Speichern des Gewichts.")
                                        } finally {
                                          setWeightEntrySaving(false)
                                        }
                                      }}
                                    >
                                      <Label htmlFor="weight-entry">Gewicht in kg</Label>
                                      <Input
                                        id="weight-entry"
                                        type="text"
                                        inputMode="decimal"
                                        ...
                                        onChange={(e) => setWeightEntryValue(e.target.value)}
                                        className="w-32 rounded-2xl border-zinc-300 bg-white text-zinc-900"
                                        disabled={weightEntrySaving}
                                      />
                                      <div className="text-xs text-zinc-500">Pflichtfeld für L-Gruppe und Wettkampfsportler nach Check-in.</div>
                                      {weightEntryError && <div className="text-sm text-red-700">{weightEntryError}</div>}
                                      <Button type="submit" className="rounded-2xl bg-[#154c83] text-white hover:bg-[#123d69] w-fit" disabled={weightEntrySaving || !weightEntryValue.trim()}>
                                        {weightEntrySaving ? "Speichert..." : "Gewicht speichern"}
                                      </Button>
                                    </form>
                                  </CardContent>
                                </Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="rounded-2xl border bg-white p-4 text-sm text-zinc-700">
                    ...
                    <span className="font-semibold">
                      {nextBaseGroupSession && typeof nextBaseGroupSession === "object" && "date" in nextBaseGroupSession && "session" in nextBaseGroupSession
                        ...
                        {safeRecentCheckins.map((row) => (
                          row ? (
                            <div key={row.id} className="flex flex-col gap-1 rounded-2xl bg-zinc-100 px-4 py-3 text-sm text-zinc-700 md:flex-row md:items-center md:justify-between">
                              ...
                              <div>
                                {row.date ? formatIsoDateForDisplay(row.date) : "—"} · {row.time || "—"}
                  ...
                  <div className="rounded-2xl border bg-white p-4">
                    <div className="mb-4 flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                      ...
                      <div className="text-sm text-zinc-500">{safeMemberAttendanceRows.length} gespeicherte Check-ins</div>
                    </div>
                    {safeMemberAttendanceRows.length === 0 ? (
                      <div className="text-sm text-zinc-500">Noch keine Anwesenheit gespeichert.</div>
                    ) : (
                      <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
                        {safeMemberAttendanceRows.map((row) => (
                          row ? (
                            <div key={`attendance-${row.id}`} className="flex flex-col gap-1 rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-700 md:flex-row md:items-center md:justify-between">
                              <div className="font-medium text-zinc-900">{row.group_name}</div>
                              <div>{row.date ? formatIsoDateForDisplay(row.date) : "—"} · {row.time || "—"}</div>
                            </div>
                          ) : null
                        ))}
                      </div>
