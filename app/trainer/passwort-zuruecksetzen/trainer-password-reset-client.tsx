...
import { TRAINER_PIN_HINT, TRAINER_PIN_REQUIREMENTS_MESSAGE, isTrainerPinCompliant } from "@/lib/trainerPin"

export function TrainerPasswordResetClient() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await fetch(`/api/public/trainer-password-reset?token=${encodeURIComponent(token)}`)
  ...
  async function requestResetLink() {
    try {
      ...
      setMessage(result.message || "Wenn ein passender Trainer existiert, wurde ein Reset-Link versendet.")
    } catch (nextError) {
      console.error(nextError)
  ...
  async function confirmReset() {
    ...
    if (!isTrainerPinCompliant(password)) {
      setError(TRAINER_PIN_REQUIREMENTS_MESSAGE)
      return
    }
    ...
    try {
      ...
      setMessage("Dein Passwort wurde aktualisiert. Du wirst gleich zum Trainer-Login weitergeleitet.")
      setNewPassword("")
      setConfirmPassword("")
            ...
            {!token ? (
              <div className="space-y-4">
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700">
                  Gib die E-Mail-Adresse deines Trainerkontos ein. Wenn ein passender Trainer mit bestätigter E-Mail existiert, senden wir einen Reset-Link.
                </div>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
                  ...
                  <div className="mt-2">
                    {tokenChecked && tokenValid && email ? `Trainerkonto: ${email}` : "Der Link wird geprüft."}
                  </div>
                </div>
